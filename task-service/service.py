...
class RabbitMQClient:
    ...
    def _send_data(self, dict_data: dict, queue_name):
    ...
    def _send_error(self, error_msg: str, queue_name):
        self.channel.queue_declare(queue=queue_name, durable=True)
    ...
    def process_message(self, ch, method, properties, body):
        ...
        if msg_type == "get_groups":
            self._send_data(logic.get_groups_for_username(data["username"]), reply_to)
        
        elif msg_type == "is_admin":
            res = logic.json_to_group_dto(data)
            if res[0]:
                self._send_data({"is_admin": logic.is_user_admin_of_group(res[1])}, reply_to)
            else:
                self._send_error(res[1], reply_to)
            ...
            if not res:
                self._send_error(users, reply_to)
            else:
                self._send_data({"users": users}, reply_to)
            ...
            if not res:
                self._send_error(id, reply_to)
            else:
                self._send_data({"group_id" : id}, reply_to)
            ...
            if logic.delete_group(res[1]):
                self._send_error("failed to delete group.", reply_to)
            else:
                self._send_data({}, reply_to)
            ...
            if logic.add_member_to_group(data["member"], res[1]):
                self._send_error("failed to add member to group.", reply_to)
            else:
                self._send_data({}, reply_to)
            ...
            if logic.delete_member_from_group(data["member"], res[1]):
                self._send_error("failed to delete member from group.", reply_to)
            else:
                self._send_data({}, reply_to)
        ...
        # Task
            ...
            if not res:
                ...
                self._send_data({"task_id": id}, reply_to)
            ...
            if not task_dto[0]:
                self._send_error(task_dto[1], reply_to)
                return
            
            group_data_dto = json_to_group_data_dto(data)
            if not group_data_dto[0]:
                self._send_error(group_data_dto[1], reply_to)
                return
            ...
            if not res:
                self._send_error(tasks, reply_to)
            else:
                self._send_data(tasks, reply_to)
            ...
            if not res:
                ...
                self._send_data(users, reply_to)
